...
df_red_wine = spark.read.format('csv')\
...
df_white_wine = spark.read.format('csv').option('inferSchema', 'true').option('sep', ';').option('header', 'true').load('/databricks-datasets/wine-quality/winequality-white.csv')

display(df_white_wine)

# COMMAND ----------

df_red_wine = df_red_wine.withColumn('wine_type', lit('red'))
df_red_wine.show

# COMMAND ----------

df_white_wine = df_white_wine.withColumn('wine_type', lit('white'))
df_white_wine.show

# COMMAND ----------

df_combined_all_wines = df_red_wine.union(df_white_wine)
display(df_combined_all_wines)

# COMMAND ----------

df_combined_all_wines_v2 = df_combined_all_wines.withColumnRenamed('quality', 'nota')
display(df_combined_all_wines_v2)

# COMMAND ----------

(
    df_combined_all_wines_v2.select(['nota', 'wine_type']).show()
)

# COMMAND ----------

(
    df_combined_all_wines_v2.groupBy(['nota', 'wine_type']).count().show()
)
...
df_combined_all_wines_v2.printSchema()

# COMMAND ----------

(
    df_combined_all_wines_v2
    .write
    .option('header', True)
    .mode('overwrite')
